                    ...
                    @foreach ($categories as $categorie)
                        <tr class="bg-white border-b">
                            <td class="px-6 py-4 category">
                                <p data-category="edit-{{ strtolower($categorie->name) }}" class="underline edit-name">{{ $categorie->name }}</p>
                                <form data-category="edit-{{ strtolower($categorie->name) }}" class="w-full align-center flex edit-form hidden" action="/admin/categories/update/" method="post">
                                    @csrf
                                    ...
                                    <div class="flex items-center justify-end ml-4 edit-submit">
                                        <p class="cursor-pointer cancel-form edit-{{ strtolower($categorie->name) }}">Annuler</p>
                                        <x-button class="ml-4">
                                            {{ __('Modifier') }}
                                        </x-button>
                                    </div>
                            ...
                            <td class="px-6 py-4 flex justify-end items-center">
                                <div class="relative inline-block text-left dropdownHover">
                                    <p id="edit-{{ strtolower($categorie->name) }}" class="hover:bg-gray-200 py-1 px-4 dropdownHover cursor-pointer edit-cta"><i class="fa fa-pen-to-square mt-0.5" style="color: #000" aria-hidden="true"></i></p>
                                </div>
                                <div class="relative inline-block text-left dropdownHover">
