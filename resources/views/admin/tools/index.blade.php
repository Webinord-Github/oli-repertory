...
@section('content')
    <div class="container" style="margin-top:75px;">
        ...
        <h1 class="text-4xl font-bold pb-6">Tools</h1>
        @if (session('status'))
            ...
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4">{{ $tool->user()->first()->name }}</td>
                        <td class="px-6 py-4">{{ $tool->status }}</td>
                        <td class="px-6 py-4">
                            <div class="relative inline-block text-left dropdownHover">
                                ...
                                <div class="absolute right-0 z-10 my-0 w-56 origin-top-right rounded-md bg-white shadow-lg focus:outline-none h-0 overflow-hidden dropdown-child dropdownHover" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                    <div class="flex flex-col dropdownHover" role="none">
                                        <a href="/admin/tools/update/{{ $tool->id }}" class="hover:bg-gray-200 py-1 px-4 dropdownHover">Edit</a>
                                        <a href="/admin/tools/destroy/{{ $tool->id }}" onclick="return confirm('Delete this tool?');" class="hover:bg-gray-200 py-1 px-4 dropdownHover">Delete</a>
                                    </div>
                                </div>
                            </div>
                        </td>
